...
- 详细卡片金额使用三种独立裁剪/预处理结果按交易单号投票；唯一多数才生成
  候选，平票保留现场但不进入自动回调。
...
详细卡片的每张不可变截图会生成三路 OCR 证据：保留原裁剪的 `150%`
放大、向左扩展金额区域后的灰度/自动拉伸/锐化，以及同一扩展区域的灰度
阈值化。解析器以微信交易单号聚合三路结果；金额和秒级时间存在唯一多数时
才生成事件，冲突平票时跳过该交易，避免把类似 `12.34` / `12.43` 的数字
顺序误读直接送入自动回调。
