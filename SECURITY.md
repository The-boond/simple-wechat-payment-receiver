...
- Existing windows and notifications form a baseline; startup does not replay historical receipts.
...
- Windows notification collection and minimized-window restoration are disabled by default.
- When notifications are enabled, the OCR helper returns only apps matching the configured allowlist.
...
Linux environment files and JSON configuration files should be readable only by the service account. On Windows, prefer a user-scoped environment variable or a protected service-secret store.

## Privacy

OCR can expose text visible in the selected WeChat window. Keep `include_raw_ocr_text`, `keep_screenshots` and `include_notifications` disabled unless diagnostics require them. Delete diagnostic captures after use.
